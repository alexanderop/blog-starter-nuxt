import Fuse from 'fuse.js'
import type { SearchResult } from '~/types/search'

export const useFuzzySearch = (searchQuery: Ref<string>) => {
  // Fetch all blog posts - get all fields including built-in ones
  const { data: allPosts } = useAsyncData('all-posts-for-fuzzy-search', () => 
    queryCollection('blog')
      .select('title', 'description', 'path', 'date', 'tags', 'body')
      .all()
  )

  const fuseInstance = computed(() => {
    if (!allPosts.value) return null
    return new Fuse(allPosts.value, {
      keys: ['title', 'description', 'tags'],
      threshold: 0.4, // Adjust for sensitivity
      distance: 100,
      minMatchCharLength: 2,
      includeScore: true,
      ignoreLocation: true,
    })
  })
  
  const { data: results, pending: isLoading } = useAsyncData(
    () => `fuzzy-search-${searchQuery.value}`,
    async () => {
      const query = searchQuery.value.trim()
      if (!query || !fuseInstance.value) {
        return []
      }

      const fuseResults = fuseInstance.value.search(query)
      
      return fuseResults
        .slice(0, 20)
        .map(result => ({
          id: result.item.path || String(Date.now() * Math.random()),
          title: result.item.title || 'Untitled',
          description: result.item.description || '',
          tags: result.item.tags || [],
          date: result.item.date || new Date().toISOString(),
          slug: result.item.path?.split('/').pop() || '',
          excerpt: result.item.description?.slice(0, 150) + '...' || '',
          content: result.item.description || '',
        })) as SearchResult[]
    },
    {
      watch: [searchQuery, fuseInstance]
    }
  )
  
  return { 
    results: computed(() => results.value || []), 
    isLoading: computed(() => isLoading.value) 
  }
}